...
class Vtiger_RelatedList_View extends Vtiger_Index_View {
	...
	function process(Vtiger_Request $request) {
		...
		$rquery = $adb->pquery("select total from vtiger_quotes q inner join vtiger_crmentity c on q.quoteid=c.crmid where c.deleted= 0 and q.quoteid=".$parentId);
		$total = $adb->query_result($rquery , 0 ,'total');

		$payquery = $adb->pquery("select SUM(payment_tks_paymenttotal) as payment from vtiger_payment p inner join vtiger_crmentity c on p.paymentid=c.crmid where c.deleted= 0 and p.payment_tks_invoiceno=".$parentId);
