...
class Webform_Capture {
	...
	function captureNow($request) {
		...
		try {
			...
			if($webform->getTargetModule() == 'Leads'){
				$response = Zend_Json::encode(array('success' => true, 'result' => 'ok', 'recordid' => $recordid[1]));
				echo $response;
			}else{
				$this->sendResponse($returnURL, 'ok', $recordid);
			}
			return;
	...
	protected function sendResponse($url, $success = false, $failure = false , $recordid = false) {
		if (empty($url)) {
			if ($success)
				$response = Zend_Json::encode(array('success' => true, 'result' => $success, 'result1' => $recordid));
			else
