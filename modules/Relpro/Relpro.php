...
class Relpro extends Vtiger_CRMEntity {
	var $table_name = 'vtiger_relpro';
	var $table_index= 'relproid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_relprocf', 'relproid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_relpro', 'vtiger_relprocf');
	...
	var $related_tables = Array( 
					'vtiger_relprocf' => Array('relproid')
					);
	...
	var $tab_name_index = Array(
		...
		'vtiger_relpro'   => 'relproid',
	    'vtiger_relprocf' => 'relproid');
	...
	var $list_fields = Array (
		...
		'Relpro No' => Array('relpro', 'relprono'),
/*'Relpro No'=> Array('relpro', 'relprono'),*/
...
/*'Relpro No'=> 'relprono',*/
	...
	var $popup_fields = Array ('relprono');

	// For Alphabetical search
	var $def_basicsearch_col = 'relprono';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'relprono';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('relprono','assigned_user_id');

	var $default_order_by = 'relprono';
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			Relpro::checkWebServiceEntry();
			Relpro::createUserFieldTable($moduleName);
			Relpro::addInTabMenu($moduleName);
	...
	static function checkWebServiceEntry() {
		...
		$sql       =  "SELECT count(id) AS cnt FROM vtiger_ws_entity WHERE name = 'Relpro'";
		...
		if($adb->num_rows($result) > 0)
		{
			...
			if($no == 0)
			{
				...
				$ws_entitySql = "INSERT INTO vtiger_ws_entity ( id, name, handler_path, handler_class, ismodule ) VALUES".
						  " (?, 'Relpro','include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation' , 1)";
