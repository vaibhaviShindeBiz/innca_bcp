	...
	{if $BLOCK_FIELDS|@count gt 0}
			...
			<div class="lineitemTableContainer">
				<table class="table table-bordered" id="lineItemTab">
					<tr>
						...
						<td><strong>{vtranslate('LBL_CTGRY',$MODULE)}</strong></td>
						<!-- New Changes Opp Module -->
		...
		<div class="fieldBlockContainer">
			<table class="table table-bordered blockContainer lineItemTable" id="lineItemResult">
				...
				<tr valign="top">
					<td width="83%">
						<span class="pull-right 1"><strong>{vtranslate('LBL_GRAND_TOTAL',$MODULE)}</strong></span>
					</td>
					<td>
				...
				{if $MODULE neq 'SalesOrder'}
					<tr valign="top">
						<td width="83%" >
							<div class="pull-right">
								{if $MODULE eq 'Invoice' or $MODULE eq 'Quotes'}
						...
						<td>
							{if $MODULE eq 'Invoice' or $MODULE eq 'Quotes'}
								<span class="pull-right"><input id="received" name="received" type="text" class="lineItemInputBox form-control" value="{if $RECORD->getDisplayValue('received') && !($IS_DUPLICATE)}{$RECORD->getDisplayValue('received')}{else}0{/if}" readonly></span>
								{else}
