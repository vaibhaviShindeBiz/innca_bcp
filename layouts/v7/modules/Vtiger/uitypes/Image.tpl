	...
	{else}
		...
		<div class="fileUploadContainer text-left">
			<div class="fileUploadBtn btn btn-primary">
				...
				<input type="file" class="inputElement {if $MODULE eq 'Products' or $MODULE eq 'Sitevisit'}multi max-20{/if} {if $FIELD_MODEL->get('fieldvalue') and $FIELD_INFO["mandatory"] eq true} ignore-validation {/if}" name="{$FIELD_MODEL->getFieldName()}[]" value="{$FIELD_MODEL->get('fieldvalue')}"
		...
		{if $FIELD_MODEL->getFieldDataType() eq 'image' || $FIELD_MODEL->getFieldDataType() eq 'file'}
			{if $MODULE neq 'Products' and $MODULE neq 'Sitevisit' }
                            <div class='redColor'>
				{vtranslate('LBL_NOTE_EXISTING_ATTACHMENTS_WILL_BE_REPLACED', $MODULE)}
                            </div>
                        {/if}
		{/if}
		{if $MODULE eq 'Products' or $MODULE eq 'Sitevisit' }<div id="MultiFile1_wrap_list" class="MultiFile-list"></div>{/if}
        
        {if $MODULE eq 'Sitevisit'}
		{foreach key=ITER item=IMAGE_INFO from=$IMAGE_DETAILS}
			<div class="row" style="margin-top:5px;">
				{if !empty($IMAGE_INFO.url) && $IMAGE_INFO.descriptionOffield eq $FIELD_MODEL->get('name')}
					{if ( strpos($IMAGE_INFO.name , '.png') >= 1 || strpos($IMAGE_INFO.name , '.jpeg') >= 1 || strpos($IMAGE_INFO.name , '.jpg') >= 1)  }
						<span class="col-lg-6" name="existingImages"><img src="{$IMAGE_INFO.location}" data-image-id="{$IMAGE_INFO.id}" width="400" height="250" ><span><a href="{$IMAGE_INFO.url}">{$IMAGE_INFO.name}</a></span></span>
	...
	{else}
			...
			<!-- ============================== lokesh ===== changes -->
				{if !empty($IMAGE_INFO.url) }
					{if ( strpos($IMAGE_INFO.name , '.png') > 1 || strpos($IMAGE_INFO.name , '.jpeg') > 1 || strpos($IMAGE_INFO.name , '.jpg') > 1)  }
						<span class="col-lg-6" name="existingImages"><img src="{$IMAGE_INFO.url}" data-image-id="{$IMAGE_INFO.id}" width="400" height="250" ><span><a href="{$IMAGE_INFO.url}">{$IMAGE_INFO.name}</a></span></span>
