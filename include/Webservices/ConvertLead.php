...
function vtws_convertlead($entityvalues, $user) {
	...
	try {
		...
		if($potentialId){
			$currentDate = explode(' ', date('Y-m-d H:i:s'));
			$task1 = Vtiger_Record_Model::getCleanInstance('Calendar');
			$task1->set('subject', 'For Customer Visit');
			$task1->set('date_start', $currentDate[0]);
			$task1->set('time_start', $currentDate[1]);
			$task1->set('due_date', $currentDate[0]);
			$task1->set('cf_1090', 'Customer Visit');
			$task1->set('activitytype', 'Task');
			$task1->set('parent_id', $potentialId);
			$task1->save();

			$task2 = Vtiger_Record_Model::getCleanInstance('Calendar');
			$task2->set('subject', 'For Designer');
			$task2->set('date_start', $currentDate[0]);
			$task2->set('time_start', $currentDate[1]);
			$task2->set('due_date', $currentDate[0]);
			$task2->set('cf_1090', 'Designer');
			$task2->set('activitytype', 'Task');
			$task2->set('parent_id', $potentialId);
			$task2->save();
		}
