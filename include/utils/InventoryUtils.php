...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='')
{
	...
	for($i=1; $i<=$tot_no_prod; $i++)
	{
		...
		$lngth_value = vtlib_purify($_REQUEST['length_value'.$i]);
		$height_value = vtlib_purify($_REQUEST['height_value'.$i]);
		...
		$product_category = $_REQUEST['product_category'.$i];
		$varients = $_REQUEST['varients'.$i];
		//New Changes Opp Module

		$query = 'INSERT INTO vtiger_inventoryproductrel(id, productid, sequence_no, quantity, listprice, comment, description, purchase_cost, margin, length_value, height_value, product_category, varients)
					VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)';
		$qparams = array($focus->id,$prod_id,$prod_seq,$qty,$listprice,$comment,$description, $purchaseCost, $margin ,$lngth_value , $height_value, $product_category, $varients);
		$adb->pquery($query,$qparams);
